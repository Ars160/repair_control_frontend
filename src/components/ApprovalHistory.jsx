            ...
            <div className="relative border-l-2 border-slate-200 ml-3 space-y-8 pb-2">
                {approvals.map((approval, index) => (
                    <div key={index} className="ml-8 relative group">
                        {/* Dot */}
                        <div className={`absolute -left-[39px] w-5 h-5 rounded-full border-4 border-slate-50 shadow-sm transform translate-y-0.5 transition-transform group-hover:scale-110 ${approval.decision === 'APPROVED' ? 'bg-emerald-500' : 'bg-rose-500'
                            }`}></div>
                        ...
                        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2 bg-white p-4 rounded-2xl border border-slate-100 shadow-sm group-hover:shadow-md transition-shadow">
                            <div>
                                <h4 className={`text-base font-bold mb-1 ${approval.decision === 'APPROVED' ? 'text-emerald-700' : 'text-rose-700'}`}>
                                ...
                                {approval.comment && (
                                    <div className={`mt-3 text-sm p-4 rounded-xl border ${approval.decision === 'APPROVED'
